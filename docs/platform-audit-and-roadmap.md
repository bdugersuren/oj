...
| P0.3 Secret/network/dependency | **Хэсэгчлэн** | Production secret/HTTPS CORS/cookie startup validation; JWT/data-at-rest key separation, previous-key migration command/runbook; unused fake DMOJ key config устсан; Compose mounted secret-file override; PostgreSQL/MinIO rotate → superseded-secret reject → data-preserving rollback drill; internal network/port hardening; capability/PID/RAM limit; бүх Dockerfile base image version+manifest digest pin болон CI gate; non-root app runtime; Git-д track болсон NPM DB/key/log index-ээс салж, named volume + loopback bootstrap болсон; pgAdmin file secret accept/reject/restart, Open WebUI persistent generated key/CORS/Ollama embedding/restart runtime smoke нэмэгдсэн | NPM/Open WebUI бодит initial admin ба signup closure, external LAN/VPN smoke, repository history rewrite ба credential/certificate rotation; read-only root/AppArmor/seccomp policy батлах |
| P0.4 CI/test baseline | **Хэсэгчлэн** | GitHub Actions backend/frontend/Compose gate; reproducible non-root backend image; 94/94 backend test pass; Docker Next.js 16 production build/TypeScript/34-route prerender pass; local core+judge full-stack healthy; submission duplicate/PubSub E2E AC; migration/Compose болон бусад security/judge smoke | Frontend 123 error/216 warning lint өр; npm audit 4 vulnerability; component/E2E/security scan gate; backend 5 warning |
...
- P0.3 Secret/network/dependency
  ...
  - [x] Бүх Dockerfile base image semantic version + registry manifest digest pin, CI unpinned-image gate
  - [x] Compose secret file ingestion ба PostgreSQL/MinIO/backend runtime smoke
  - [x] PostgreSQL/MinIO credential rotation, superseded-secret rejection, rollback/recovery drill
  - [x] API JWT/data-at-rest key separation, previous-key migration/runbook, unused DMOJ key removal
  - [x] NPM runtime DB/key/log-ийг Git index-ээс салгаж ignore хийх, named volume ба loopback-only bootstrap profile
  - [x] pgAdmin password file secret, persistent volume; Open WebUI generated persistent signing key; admin bootstrap/rotation runbook
  - [x] NPM legacy default reject/loopback binding; pgAdmin correct/wrong credential + restart; Open WebUI persistent key/CORS/Ollama embedding + restart runtime smoke
  - [ ] NPM/Open WebUI initial admin, signup closure, external LAN/VPN exposure smoke; Git history rewrite ба бодит credential/certificate rotation
  - [ ] Read-only root, writable-path inventory, AppArmor/seccomp policy
- P0.4 CI/test baseline
  - [x] Backend/frontend/Compose GitHub Actions baseline
  - [x] Backend 94/94 test, fresh/downgrade/upgrade migration, live/ready/WebSocket snapshot tests
  - [x] Python compile, TypeScript typecheck, Compose profile, diff validation
  - [x] DMOJ socket verdict smoke matrix
  - [x] Docker Next.js production build, TypeScript, 34-route prerender
  - [ ] Frontend ESLint zero-error
...
- `docker-compose.secrets.yml` production override нь API/migrate/Celery-ийн `.env` inheritance-ийг reset хийж, JWT, data encryption key, database URL болон MinIO credential-ийг `/run/secrets/*` read-only file-аар өгнө. PostgreSQL/MinIO direct password environment-ийг Compose merge-ээс устгаж, албан `_FILE` contract ашиглана; `.env` болон `secrets/` version control-оос хасагдсан.
- Backend startup нь `SECRET_KEY`, `ENCRYPTION_KEY`, `DATABASE_URL`, `REDIS_URL`, MinIO credential, SMTP password-ийн `*_FILE` хувилбарыг уншина. File байхгүй/хоосон эсвэл process environment-д direct утга давхар өгөгдвөл fail-fast. Rendered production Compose-д direct secret value байхгүй, PostgreSQL болон MinIO file-only startup smoke pass болсон.
- JWT signing `SECRET_KEY` болон DB data-at-rest `ENCRYPTION_KEY` салсан. Production explicit, хамгийн багадаа 32 тэмдэгт key-гүй асахгүй; previous key мөн хүчтэй, current key-ээс ялгаатай байх ёстой. JWT rotate хийхэд хуучин access token reject боловч SMTP ciphertext decrypt хэвээр байх regression тесттэй. Encryption-key rotation үед `ENCRYPTION_KEY_PREVIOUS` түр read-only mount-аар хуучин ciphertext уншиж, `rotate_encrypted_settings.py` зөвхөн амжилттай decrypt/re-encrypt болсон үед commit хийнэ. Дараалал, rollback, key removal-ийг `docs/operations/secret-rotation.md`-д баримтжуулсан.
- `DMOJ_JUDGE_KEY` болон hardcoded `judge/bridge/bridge.yml` нь custom socket bridge runtime-д огт уншигддаггүй dead config байсан тул хамгаалалт мэт буруу ойлголт төрүүлэхээс сэргийлж устгасан. Бодит bridge trust boundary нь outbound-гүй internal `judge-network`, API/Celery access болон socket payload validation хэвээр.
- `smoke_secret_rotation.sh` нь тогтмол production container name-ийг reset хийсэн тусдаа Compose project, random `0600` secret, тусдаа PostgreSQL/MinIO volume үүсгэнэ. Forward rotation дараа шинэ credential accepted, хуучин credential rejected, marker data preserved; rollback дараа хуучин credential accepted, шинэ credential rejected, marker data preserved болсон. Эцэст нь container/network/volume/secret fixture бүгд автоматаар устсан.
...
- Docker backend test image дотор 94 test амжилттай, 5 warning үлдсэн. JWT expiry timezone-aware, DB refresh expiry timezone-naive UTC schema-тай compatible хэвээр. Docker Next.js 16.3 production build compile/TypeScript/34 route prerender бүгд pass; middleware→proxy deprecation warning үлдсэн. Frontend lint baseline 123 error/216 warning, image build үеийн `npm audit` 1 low/1 moderate/2 high vulnerability мэдээлсэн.
- `docker-compose.local.yml` нь Web/API-г зөвхөн `127.0.0.1:3000/8000` дээр гаргаж, browser bundle-ийн API origin-ийг build arg-аар localhost API руу тохируулна. NPM/AI/admin-tools-гүй core+judge stack дээр PostgreSQL/Redis/MinIO/API/хоёр bridge healthy, migration exit 0, Celery ready, frontend/API HTTP 200 болсон; cookie/CSRF/RBAC/refresh/WebSocket actual ASGI smoke мөн AC.
- Ажиллаж буй local stack дээр submission → Celery → DMOJ → PostgreSQL/Redis PubSub E2E `AC`; concurrent duplicate delivery ignore, reward/JudgeResult давхардаагүй. Smoke нь seeded `FIRST_AC` 50 XP bonus-ийг динамикаар тооцож, хоёр healthy bridge болон injected dead-bridge горимыг ялгадаг, үүсгэсэн user/problem/submission/level fixture-ээ `finally` цэвэрлэдэг болсон.
- `config/npm/data` доторх `database.sqlite`, `keys.json`, proxy configuration болон 13MB орчим access log Git-д track болсон Critical олдвор илэрсэн. Агуулгыг аудитын output-д задлаагүй; working copy-г устгалгүй `git rm --cached`-аар index-ээс салгаж, runtime path-ийг `.gitignore`-д оруулсан. Энэ нь хуучин Git history-г цэвэрлэхгүй тул repository shared болсон эсэхийг тогтоож history rewrite, NPM account/JWT/access-list credential болон хамаарах certificate/private key rotation хийх хүртэл incident нээлттэй.
- NPM state bind mount-оос named volume руу шилжиж `ops` profile-д орсон. `docker-compose.admin-bootstrap.yml` эхний асаалтад 80/81/443-г зөвхөн loopback-д bind хийнэ; reviewed fresh state болон default credential солигдсоны дараа л steady-state public 80/443-г нээх хоёр шаттай runbook нэмэгдсэн. Хуучин SQLite/key файлыг шинэ volume руу автоматаар зөөгөөгүй.
- pgAdmin default email/password fallback арилж, production password зөвхөн `/run/secrets/pgadmin_default_password`-оос уншина; localhost 5050, persistent volume, capability drop/no-new-privileges/PID/RAM limit-тэй болсон. Open WebUI-ийн хоосон `WEBUI_SECRET_KEY` injection арилж, 32 random byte key-г `openwebuidata`-д `0600` mode-оор үүсгэн official file contract-аар уншина; UI зөвхөн localhost 3001, CORS explicit origin-той. Compose-ийн core/judge/ai/ops/admin/bootstrap render pass; production override нь `CORS_ORIGINS` хоосон үед хүлээгдсэнээр fail-closed, dummy HTTPS origin-той render pass болсон.
- NPM 2.15 fresh named volume/loopback bootstrap дээр HTTP 200, бүх 80/81/443 mapping зөвхөн `127.0.0.1`; legacy `admin@example.com/changeme` token request 400 болж reject, API initial setup incomplete гэж мэдээлсэн. pgAdmin 9.17 дээр unreadable `0600 uid=1000` bind-secret startup-ийг fail-closed зогсоосон; protected parent дотор container-readable file болгосны дараа bootstrap user үүсч, gunicorn 8080 дээр асан `/misc/ping` 200 болсон. Secret plaintext container environment-д байгаагүй, localhost 5050 binding болон cap-drop/no-new-privileges/PID/RAM limit inspect-ээр батлагдсан; browser login accept/reject ба restart persistence үлдсэн.
- pgAdmin-ийн өөрийн Flask-Security verifier дээр mounted current password hash `true`, deliberately wrong password `false`; restart дараа `/misc/ping` 200 болон active bootstrap user count 1 хэвээр. Open WebUI-ийн анхны key image writable layer-д 17 byte/`0644` үүсэж байсныг runtime smoke илрүүлж, local entrypoint-аар persistent data volume дахь 32 random byte/base64 45-byte `0600` file болгосон. Restart өмнө/дараа key digest ижил, `/health` 200/healthy, `WEBUI_SECRET_KEY` plaintext environment-д байгаагүй. Default CORS `*` warning арилж explicit origin болсон; default Hugging Face startup download-ийг internal Ollama embedding engine болгосноор model download-гүй ready болсон. Ollama-д embedding model байхгүй бол document/RAG ready биш хэвээр.
- Admin smoke дууссаны дараа NPM, pgAdmin, Open WebUI контейнерийг volume-ийг нь хадгалан зогсоосон; түр password/cookie/payload fixture устсан. Core+judge stack зогсоогүй, API/DB/Redis/MinIO/хоёр bridge healthy, Web/Celery running хэвээр.
- MinIO, NPM, Ollama, Open WebUI, pgAdmin, Qdrant-ын version tag бүрийг registry-ийн multi-architecture manifest digest-тэй lock хийсэн. Ингэснээр бүх Dockerfile-ийн remote `FROM` одоо human-readable version + immutable `@sha256` хослолтой; CI Compose job unpinned base image дахин нэмэгдэхэд fail хийх gate-тэй болсон.

### Яг дараагийн алхам

1. `docs/operations/admin-tools.md`-ийн localhost/SSH tunnel горимоор owner өөрийн NPM 2.15 initial admin болон Open WebUI first admin-ийг үүсгэж, Open WebUI signup-г UI-д хаана; credential-ийг код/log-д дамжуулахгүй. Дараа нь private browser болон өөр LAN machine-аас signup/admin-port negative smoke хийнэ.
2. Ollama-д `OPEN_WEBUI_EMBEDDING_MODEL`-ийг татаж checksum/size-г тэмдэглэн document/RAG smoke хийнэ; AI profile ажиллахгүй үед core submission/contest хэвийн гэдгийг дахин батална.
3. Repository remote/shared exposure-г incident owner-той тогтоож Git history rewrite хийх; дараа нь NPM admin/JWT/access-list credential болон repository-д орсон certificate/private key-г rotate/reissue хийгээд нотолгоог incident record-д хадгалах.
4. Service бүрийн writable-path inventory гаргаж, эхлээд Web/API/Celery-г `read_only` + bounded `tmpfs`-тэй runtime smoke хийх; дараа нь DB/storage/admin/judge-д тусгай AppArmor/seccomp policy-г шаталж батлах.
5. Frontend lint error болон npm audit vulnerability-г багцаар цэвэрлээд component/E2E/security scan gate ажиллуулах.
6. Generator-ийн per-parameter compile болон 50 concurrent submission load budget хэмжих; queue wait/lease/visibility timeout-ийг хэмжилтээр тохируулах.
...
Аудит эхлэх үед системийг интернетэд production хэлбэрээр нээх боломжгүй байсан гол шалтгаан нь sample run болон judge унтарсан горимд сурагчийн код API/Celery контейнерийн дотор энгийн `subprocess`-оор ажилладаг явдал байв. Энэ замыг одоогийн working tree-д хааж, submission/sample/model-solution/generator-ийг DMOJ queue boundary руу шилжүүлсэн. Submission queue, dead-bridge retry, hard-kill reclaim, workspace full-stack E2E, archive endpoint integration, advanced grader correctness, storage credential rotation болон API key separation батлагдсан ч admin-tool credential policy, үлдсэн container hardening, frontend quality gate бүрэн дуусаагүй тул **P0 production hold одоогоор хэвээр**.

Одоо үлдсэн том эрсдэлүүд нь admin-tool credential lifecycle, бүх image-ийн immutable pin, үлдсэн container policy, backup/restore, observability болон frontend quality debt юм. Host exposure, mock AC, infrastructure→RTE, worker hard-kill recovery, workspace full-stack pipeline, advanced grader, PostgreSQL/MinIO secret rotation, API key separation, actual auth/WebSocket болон malicious upload boundary зэрэг анхны P0 олдворын хэд хэдэн хэсэг working tree-д засагдсан. Frontend lint baseline 123 error/216 warning; backend reproducible image suite 94 pass болсон.
...
- Анхны аудитад host `pytest` байгаагүй; одоо reproducible backend image дотор 94 test pass болсон. PostgreSQL fresh bootstrap, downgrade болон өмнөх Alembic head-ээс upgrade smoke pass болсон.
...
| S-05 | **Critical** | NPM-ийн SQLite DB, `keys.json`, proxy config болон access/error log repository-д track болсон | Admin/password hash ба JWT/private material leak, домэйн/IP/route disclosure; Git history-оор удаан хадгалагдах | Index-ээс салгах, history rewrite; бүх холбогдох credential/key/certificate-г compromised гэж үзэн rotate/reissue; fresh named volume | P0 |
